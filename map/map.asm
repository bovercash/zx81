...
; 24 lines, 32 bytes, each seperated by a NL byte
...
PRINT_TITLE_LOOP:
	...
	LD A, (HL)
	CP $FF
	JR NZ, PRINT_TITLE_LOOP

	; init the program
	LD HL, MAP_ONE
	LD (CURRENT_MAP), HL	; set the current map to map one 

	LD HL, (D_FILE)		; screen pointer
	LD DE, 34
	ADD HL, DE
	LD (D_FILE_INDEX), HL	; this should be the address of where to start
...
CURRENT_MAP	DW MAP_ONE	; Map Pointer
D_FILE_INDEX	DW display_file	; Pointer to current location in D_FILE

; Text and Labels
title_msg	DB _M,_A,_P,_SPC,_D,_E,_M,_O,$FF	
